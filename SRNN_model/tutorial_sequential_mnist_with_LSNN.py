...
from toolbox.tensorflow_einsums.einsum_re_written import einsum_bij_jk_to_bik
...
from toolbox.file_saver_dumper_no_h5py import save_file, get_storage_path_reference
from tutorial_sequential_mnist_plot import update_mnist_plot

from model import tf_cell_to_savable_dict, exp_convolve, ALIF, LIF
from toolbox.rewiring_tools import weight_sampler, rewiring_optimizer_wrapper
from toolbox.tensorflow_utils import tf_downsample
import json
from tensorflow.examples.tutorials.mnist import input_data

FLAGS = tf.compat.v1.flags

## tf.compat.v1.flags.DEFINE_xxx -- Registers a flag whose value can be any xxx.
tf.compat.v1.flags.DEFINE_string('comment', '', 'comment to retrieve the stored results')
tf.compat.v1.flags.DEFINE_string('resume', '', 'path to the checkpoint of the form "results/script_name/2018_.../session"')
tf.compat.v1.flags.DEFINE_string('model', 'LSNN', 'spiking network model to use: LSNN or LIF')
tf.compat.v1.flags.DEFINE_string('save_data', True, 'whether to save simulation data in result folder')
tf.compat.v1.flags.DEFINE_string('downsampled', False, 'whether to use the smaller downsampled mnist dataset of not')
##
tf.compat.v1.flags.DEFINE_string('n_batch_train', 256, 'size of the training minibatch')
tf.compat.v1.flags.DEFINE_string('n_batch_validation', 256, 'size of the validation minibatch')
tf.compat.v1.flags.DEFINE_string('n_in', 80, 'number of input units')
tf.compat.v1.flags.DEFINE_string('n_regular', 120, 'number of regular spiking units in the recurrent layer')
tf.compat.v1.flags.DEFINE_string('n_adaptive', 100, 'number of adaptive spiking units in the recurrent layer')
tf.compat.v1.flags.DEFINE_string('reg_rate', 10, 'target firing rate for regularization')
tf.compat.v1.flags.DEFINE_string('n_iter', 36000, 'number of training iterations')
tf.compat.v1.flags.DEFINE_string('n_delay', 10, 'maximum synaptic delay')
tf.compat.v1.flags.DEFINE_string('n_ref', 5, 'number of refractory steps')
tf.compat.v1.flags.DEFINE_string('lr_decay_every', 2500, 'decay learning rate every lr_decay_every steps')
tf.compat.v1.flags.DEFINE_string('print_every', 400, 'frequency of validation')
tf.compat.v1.flags.DEFINE_string('ext_time', 1, 'repeat factor to extend time of mnist task')
##
tf.compat.v1.flags.DEFINE_float('beta', 1.8, 'Scaling constant of the adaptive threshold')
# to solve a task successfully we usually set tau_a to be close to the expected delay / memory length needed
tf.compat.v1.flags.DEFINE_float('tau_a', 700, 'Adaptation time constant')
tf.compat.v1.flags.DEFINE_float('tau_v', 20, 'Membrane time constant of output readouts')
tf.compat.v1.flags.DEFINE_float('thr', 0.01, 'Baseline threshold voltage')
tf.compat.v1.flags.DEFINE_float('learning_rate', 1e-2, 'Base learning rate')
tf.compat.v1.flags.DEFINE_float('lr_decay', 0.8, 'Decaying factor')
tf.compat.v1.flags.DEFINE_float('reg', 1e-1, 'regularization coefficient to target a specific firing rate')
tf.compat.v1.flags.DEFINE_float('rewiring_temperature', 0., 'regularization coefficient')
tf.compat.v1.flags.DEFINE_float('proportion_excitatory', 0.75, 'proportion of excitatory neurons')
##
tf.compat.v1.flags.DEFINE_bool('interactive_plot', False, 'Perform plotting')
tf.compat.v1.flags.DEFINE_bool('verbose', True, 'Print many info during training')
tf.compat.v1.flags.DEFINE_bool('neuron_sign', True,
                         "If rewiring is active, this will fix the sign of neurons (Dale's law)")
tf.compat.v1.flags.DEFINE_bool('crs_thr', True, 'Encode pixels to spikes with threshold crossing method')
# With simple grid search we found that setting rewiring to 12% yields optimal results
tf.compat.v1.flags.DEFINE_float('rewiring_connectivity', 0.12, 'max connectivity limit in the network (-1 turns off DEEP R)')
tf.compat.v1.flags.DEFINE_float('l1', 1e-2, 'l1 regularization used in rewiring (irrelevant without rewiring)')
tf.compat.v1.flags.DEFINE_float('dampening_factor', 0.3, 'Parameter necessary to approximate the spike derivative')
...
# probably for tf 2? -- mnist = tf.keras.datasets.mnist.load_data()
...
# Generate input 
# tf.placeholder is for tf1, is it necessary for tf2? Probably should delete these two lines?
...
def get_data_dict(batch_size, type='train'):
    # Probably should rewrite this function for tf2?
    """
    ...
    target_num = np.argmax(target_oh, axis=1) # Returns the indices of the maximum values of array 'target_oh' along axis 1.

    if FLAGS.ext_time > 1:
        input_px = np.repeat(input_px, FLAGS.ext_time, axis=1) # Repeat elements of array 'input_px' for 'FLAGS.ext_time' times.
